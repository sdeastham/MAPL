...
!  ---------------------------------------------------
   ...
   IF ( firstRun ) THEN
      call lgr%info('ExtData First Run_(): Start')
      call lgr%info('ExtData First Run_(): READ_LOOP: Start')
   ELSE
      call lgr%debug('ExtData Run_(): Start')
   ...
   call lgr%debug('ExtData Run_: ---PopulateBundle')
   ...
   call lgr%debug('ExtData Run_: ---prefetch')
   ...
   ! GCHP: extra prints
   call lgr%debug('ExtData Run_: ---IclientDone')
   ...
   ! GCHP: extra prints
   call lgr%debug('ExtData Run_: ---read-prefetch')
   ...
   call lgr%debug('ExtData Run_: Vertical interpolation')
   ...
   ! GCHP: always print if first run
   IF ( firstRun ) THEN
      call lgr%info('ExtData First Run_: INTERP_LOOP: Done')
   ELSE
   ...
   ! GCHP: extra prints
   IF ( firstRun ) THEN
      call lgr%info('ExtData First Run_: Calculating derived fields')
   ELSE
      call lgr%debug('ExtData Run_: Calculating derived fields')
   ENDIF
   ...
   ! GCHP: always print if first run
   IF ( firstRun ) THEN
      call lgr%info( 'ExtData First Run_: End' )
   Else
     ...
     subroutine GetBracketTimeOnSingleFile(fdata,tSeries,cTime,bSide,UniFileClim,UniFileDOW,interpTime,fileTime,tindex,allowExtrap,climyear,rc)
        ...
        If (UniFileDOW) Then
           ...
           elseif (nsteps == 84) then
              ...
              call lgr%error('DOW file %a should have either 7 or 84 entries, but instead contains %i8',trim(fdata%get_file_name()),nsteps)
              rc = ESMF_FAILURE
              return
           end if
...
! ----------
 ...
 END SUBROUTINE MAPL_ExtDataExtractIntegers
  ...
  call lgr%debug('   --> MAPL_ExtDataFillField: filling %i4 level input to %i4 levels for %a',lm_in,lm_out,trim(item%name))
  ...
  subroutine MAPL_ExtDataFlipVertical(item,filec,rc)
      ...
      call lgr%debug('   --> MAPL_ExtDataFlipVertical: vertically flipping all levels for %a', trim(item%name))
